...
class Lab1:
    ...
    def create_file1(self):
        with open("file1.txt", "w") as file:
            for i in range(10240):
                file.write(random.choice(self.symbols))
            file.close()

    def create_file2(self):
        with open("file2.txt", "w") as file:
            for i in range(10240):
                file.write(random.choices(self.symbols, weights=[0.2, 0.3, 0.5])[0])
            file.close()

    def create_file3(self):
    ...
    def shennon(self, probabilities):
        res = 0.0
        for item in probabilities:
            res -= item * math.log2(item)
        return res

    def firstProbability(self, file_name):
        ...
        entropy = self.shennon(prob_list)
        print("First probability for " + file_name + ": " + str(entropy))

    def secondProbability(self, file_name, alphabet):
        file = open(file_name, "r")
        text = file.read()
        file.close()
        all_probabilities = {}
        ...
        for item in all_probabilities:
            prob_list.append(all_probabilities[item])
        entropy = self.shennon(prob_list) / 2
        print("Second probability for " + file_name + ": " + str(entropy))

    def thirdProbability(self, file_name, alphabet):
        file = open(file_name, "r")
        text = file.read()
        file.close()
        all_probabilities = {}
        ...
        for item in all_probabilities:
            prob_list.append(all_probabilities[item])
        entropy = self.shennon(prob_list) / 3
        print("Third probability for " + file_name + ": " + str(entropy))

    def run(self):
        self.firstProbability("file1.txt")
        self.firstProbability("file2.txt")
        self.firstProbability("file3.txt")

        self.secondProbability("file1.txt", self.symbols)
        self.secondProbability("file2.txt", self.symbols)
        self.secondProbability("file3.txt", self.alphabet)

        self.thirdProbability("file1.txt", self.symbols)
        self.thirdProbability("file2.txt", self.symbols)
        self.thirdProbability("file3.txt", self.alphabet)
